...
import { registerDcEffectsMaterials } from '../dc-effects-lib'
import {
  createDcEffectRingEntity,
  updateDcEffectRingEntity,
  removeDcEffectRingEntity
} from '../dc-effects-lib'
import circleScanUrl from './circleScan.png'
...
const ENTITY_ID = 'dc-effect-radar-pic'
const RING_TYPE = 'radar-pic'
...
function makeImageMaterial(): ImageMaterialProperty {
  return new ImageMaterialProperty({
    image: circleScanUrl,
    color: Color.fromCssColorString(colorHex.value).withAlpha(0.8),
    transparent: true
  })
}
...
function place(lon: number, lat: number): void {
  ...
  const entity = viewer.entities.add({
    ...
    ellipse: {
      ...
      height: height.value,
      material: makeImageMaterial(),
      stRotation: new CallbackProperty(() => {
        stRotation += rotateAmount.value
        if (stRotation >= 360 || stRotation <= -360) stRotation = 0
  ...
  if (!entity) return
  syncRing()
  resultMessage.value = `雷达图片已放置于 ${lon.toFixed(4)}, ${lat.toFixed(4)}`
}

function syncRing(): void {
  if (!viewer || viewer.isDestroyed()) return
  removeDcEffectRingEntity(viewer, RING_TYPE)
  if (!showRing.value) return
  const entity = viewer.entities.getById(ENTITY_ID)
  if (!entity) return
  const pos = (entity as { position: { getValue: (time: unknown) => Cartesian3 | undefined } }).position.getValue(viewer.clock.currentTime)
  if (!pos) return
  ...
  createDcEffectRingEntity(viewer, {
    ...
    speed: rotateAmount.value !== 0 ? Math.abs(rotateAmount.value) : 3,
    repeat: 40,
    thickness: 0.2
  })
}
...
function updateEffect(): void {
  ...
  const ellipse = (entity as { ellipse?: { material?: ImageMaterialProperty; semiMajorAxis?: unknown; semiMinorAxis?: unknown; height?: unknown } }).ellipse
  if (!ellipse) return
  const mat = ellipse.material
  if (mat) {
    mat.color = new ConstantProperty(Color.fromCssColorString(colorHex.value).withAlpha(0.8))
  }
  ellipse.semiMajorAxis = new ConstantProperty(radius.value)
  ellipse.semiMinorAxis = new ConstantProperty(radius.value)
  ellipse.height = new ConstantProperty(height.value)
  updateDcEffectRingEntity(viewer, RING_TYPE, {
    radius: radius.value,
    height: height.value,
    color: Color.fromCssColorString(colorHex.value).withAlpha(0.9),
    speed: rotateAmount.value !== 0 ? Math.abs(rotateAmount.value) : 3
  })
  resultMessage.value = '雷达图片参数已更新'
}
...
watch([colorHex, rotateAmount, radius, height], () => {
...
<template>
  <div class="radar-pic-shell">
    <div ref="container" class="cesium-container"></div>

    <div class="control-panel">
      <div class="panel-title">雷达图片效果</div>
      ...
      <div class="control-row">
        <span class="row-label">旋转速度</span>
        <input v-model.number="rotateAmount" type="range" min="-6" max="6" step="0.5" />
        <span class="row-value">{{ rotateAmount.toFixed(1) }}</span>
      </div>
      <div class="control-row">
      ...
      <p class="hint">开启放置后单击地图定位雷达；旋转速度正值为顺时针、负值为逆时针。</p>
...
<style scoped>
.radar-pic-shell { position: relative; width: 100%; height: 100%; min-height: 320px; overflow: hidden; border-radius: 8px; background: #152b4c; }
