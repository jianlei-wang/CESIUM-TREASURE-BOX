...
import { GoogleImageryProvider, type GoogleStyle } from '../../lib/map-providers'

type Plan = { id: string; label: string; styles: GoogleStyle[]; desc: string }
const PLANS: Plan[] = [
  {
    id: 'elec',
    label: '方案一·电子地图',
    styles: ['elec'],
    desc: '原样例：谷歌电子地图（style=elec，lyrs=m）'
  },
  {
    id: 'img-cva',
    label: '方案二·影像+注记',
    styles: ['img', 'cva'],
    desc: '新增方案：卫星影像图层上叠加中文注记图层（lyrs=s + lyrs=h）'
  }
]
...
function apply(): void {
  ...
  const providers = plan.styles.map(
    (style) => new GoogleImageryProvider({ style, crs: crs.value })
  )
...
<template>
  <div class="map-shell">
    ...
    <div class="control-panel">
      <div class="panel-title">谷歌地图底图</div>
      <div class="hint">谷歌瓦片服务，走 gac-geo.googlecnapps.cn 中国节点。</div>
