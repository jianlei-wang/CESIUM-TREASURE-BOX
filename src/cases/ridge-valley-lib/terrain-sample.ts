...
const MAX_POINTS_DEFAULT = 40000
...
export function gridFromBoundsMeters(
  ...
  let cell = Math.max(MIN_CELL_METERS, cellMeters)
  let adjusted = false
  for (let attempt = 0; attempt < 40; attempt += 1) {
    ...
    if (width * height <= maxPoints) {
      return { width, height, points: width * height, cellMeters: cell, cellLon, cellLat, adjusted }
    }
    cell *= 1.15
    adjusted = true
  }
...
export async function sampleTerrainDem(
  ...
  const spec = gridFromBoundsMeters(bounds, options.cellMeters, options.maxPoints)
  ...
  const adjustNote = spec.adjusted ? '（受点数上限自动放宽）' : ''
  const source = `真实地形采样 ${spec.width}x${spec.height} · 间距约 ${spacing}m${adjustNote} · 填充 ${filled} 个无效点`
