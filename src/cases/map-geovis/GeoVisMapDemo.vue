...
import { GeoVisImageryProvider, type GeoVisStyle } from '../../lib/map-providers'

type Plan = { id: string; label: string; styles: GeoVisStyle[]; desc: string }
const PLANS: Plan[] = [
  {
    id: 'vec',
    label: '方案一·矢量地图',
    styles: ['vec'],
    desc: '原样例：星图矢量底图（style=vec）'
  },
  {
    id: 'img',
    label: '方案二·卫星影像',
    styles: ['img'],
    desc: '新增方案：星图卫星影像（style=img）'
  }
]
...
function apply(): void {
  ...
  const providers = plan.styles.map(
    (style) => new GeoVisImageryProvider({ style, key: apiKey.value.trim() })
  )
  scene.applyProviders(providers)
  loaded.value = true
  statusMessage.value = `已加载「${plan.label}」（token 后 6 位 ${apiKey.value.trim().slice(-6) || '为空'}）`
  scene.flyToLonLat(116.391, 39.907, 30000)
}

function ensureLoaded(): void {
  if (!apiKey.value.trim()) {
    statusMessage.value = '请先输入星图 token，再点击「加载底图」'
    loaded.value = false
    return
  }
  apply()
...
onMounted(() => {
  scene.mount()
  statusMessage.value = '星图需要 token。请到 https://www.geovisearth.com 申请，输入后点击「加载底图」'
})
...
<template>
  <div class="map-shell">
    ...
    <div class="control-panel">
      <div class="panel-title">星图地图底图</div>
      <div class="hint">星图瓦片服务，token 以参数形式手动输入后再加载场景。</div>

      <div class="section-title">Key 配置</div>
      <div class="control-row key-row">
        <input v-model="apiKey" class="key-input" type="password" placeholder="请输入星图 token" />
      </div>
