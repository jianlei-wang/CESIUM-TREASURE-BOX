...
import { DURATION, SLIDE_STAGES } from '../geo-hazard-lib/model'
import { LANDSLIDE_KNOWLEDGE } from '../geo-hazard-lib/knowledge'
import { LandslideThreeScene } from './LandslideThreeScene'
...
let scene: LandslideThreeScene | null = null

const steps = SLIDE_STAGES
const speeds = [0.5, 1, 2, 4]
const knowledge = LANDSLIDE_KNOWLEDGE
...
const stageSub = ['蠕动变形', '滑动破坏', '高速滑移', '减速堆积']
const stageTxt = [
  '坡体在主滑面附近缓慢蠕滑，后缘拉张裂缝张开、地面下错，坡脚渗水，树木开始歪斜。此阶段可持续数天至数年，是最宝贵的预警窗口期。',
  '裂缝贯通、滑带抗剪强度被克服，滑体剪出后整体启动并迅速加速，坡体质心明显下移。',
  '滑体沿滑动面高速下滑，前端受阻破碎、解体为碎屑流，能量释放达到峰值。',
  '滑体推挤堆积于坡脚，形成滑坡舌与鼓张裂缝，运动逐渐停止并趋于新的稳定状态，也可能在后期复活。'
]
...
onMounted(() => {
  ...
  scene = new LandslideThreeScene(canvasHost.value, { onProgress })
})
...
const legend = [
  ...
  { color: '#8a6a42', label: '滑坡体（滑动岩土体）' },
  { color: '#e0b64a', label: '滑动面 / 滑带' },
  { color: '#8c8579', label: '滑坡壁（裸露基岩）' },
  { color: '#b3a07e', label: '解体碎屑 / 堆积体' },
  { color: '#4fa8e0', label: '降雨 / 地表水流' },
  ...
  <div class="fy-shell">
    ...
    <div class="control-panel">
      <div class="panel-title-row">
        <span class="panel-title">滑坡形成与运动 3D 演示</span>
    ...
    <div v-if="infoOpen" class="route-overlay" @click.self="infoOpen = false">
      <div class="route-modal">
        <div class="route-head">
          <span class="route-title">滑坡科普知识</span>
