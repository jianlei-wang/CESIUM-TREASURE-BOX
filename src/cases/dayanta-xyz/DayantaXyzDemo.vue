...
const rotY = ref(0)
const rotZ = ref(0)
const axisLength = ref(900)
const ringRadius = ref(550)
...
// 大雁塔模型锚点：加载时其几何中心（ECEF），作为模型本地原点，基座固定取该点 ENU
let nativeP0 = new Cartesian3()
let modelRadius = 300

let currentCenter = new Cartesian3()
...
// 基座固定为模型加载时的 ENU（模型本地体轴方向），拖拽/参数修改全程不再随中心点变化
function computeBasisAtP0(): void {
  const m = Transforms.eastNorthUpToFixedFrame(nativeP0)
...
// 模型姿态 = T(中心)·R·T(-锚点)：平移只改中心、旋转只改 R，二者解耦
function updateModelTransform(): void {
  if (!viewer || viewer.isDestroyed() || !tileset) return
  const toCenter = Matrix4.fromTranslation(currentCenter, new Matrix4())
  const rotate = Matrix4.fromRotationTranslation(rotMatrix, Cartesian3.ZERO, new Matrix4())
  const fromNative = Matrix4.fromTranslation(Cartesian3.negate(nativeP0, new Cartesian3()), new Matrix4())
  const m = Matrix4.multiply(toCenter, rotate, new Matrix4())
  Matrix4.multiply(m, fromNative, m)
  tileset.modelMatrix = m
  viewer.scene.requestRender()
}

function syncCenterFromCartesian(): void {
...
function axisTargetOf(id: string): { kind: DragKind; axis: AxisName } | null {
  const m = id.match(/^xyzm-(line|handle|label|ring)-([xyz])$/)
  if (!m) return null
...
function logLine(kind: 'start' | 'move' | 'end', translateMeters: number, rotateDeg: number): void {
  ...
  const message = `[大雁塔XYZ编辑] ${head}，中心点：${centerText()}，姿态：${attitudeText()}`
  console.log(message)
  lastLogText.value = message.replace(/^\[大雁塔XYZ编辑\]\s*/, '')
}
...
function flyToModel(): void {
  if (!viewer) return
  const spread = Math.max(modelRadius, axisLength.value, ringRadius.value)
  const height = Math.max(3000, spread * 2.6 + modelRadius * 2)
  viewer.camera.flyTo({
...
function applyCenter(): void {
  ...
  updateModelTransform()
  flyToModel()
  flashStatus('模型中心点已更新，编辑坐标轴已随模型移动')
}
...
function resetAttitude(): void {
  ...
  refreshAxisStyle()
  updateModelTransform()
  flashStatus('模型姿态已重置为初始方向')
}

function resetTransform(): void {
  if (!viewer || viewer.isDestroyed()) return
  if (!requireModel()) return
  currentCenter = Cartesian3.clone(nativeP0)
  syncCenterFromCartesian()
  rotX.value = 0
  rotY.value = 0
  rotZ.value = 0
...
async function loadModel(): Promise<void> {
  ...
  try {
    ...
    nativeP0 = Cartesian3.clone(bs.center)
    modelRadius = Math.max(1, bs.radius)
    currentCenter = Cartesian3.clone(nativeP0)
    syncCenterFromCartesian()
    computeBasisAtP0()
    rotMatrix = Matrix3.IDENTITY.clone()
    refreshDirs()
    const r = modelRadius
    axisLength.value = Math.max(150, Math.min(4000, Math.round((r * 2.2) / 50) * 50))
    ringRadius.value = Math.max(100, Math.min(2600, Math.round((r * 1.4) / 50) * 50))
    AXIS_NAMES.forEach((name) => { createAxis(name) })
    createOrigin()
    refreshAxisStyle()
    updateModelTransform()
    syncLive()
...
onMounted(async () => {
  ...
  const sceneCallbacks: SceneCallbacks = {
    ...
    onBasemapReady: () => { statusMessage.value = '正在加载 Cesium World Terrain…' }
  }
  try {
...
<template>
  <div class="dayanta-xyz-shell">
    <div ref="container" class="cesium-container"></div>

    <div class="control-panel">
      <div class="panel-title">大雁塔模型XYZ编辑坐标轴-平移/旋转</div>

      <div class="section-title">模型</div>
      <div class="control-row">
        <span class="row-label">显示大雁塔模型</span>
        <label class="switch">
          <input :checked="showModel" type="checkbox" @change="(e) => { showModel = (e.target as HTMLInputElement).checked; onShowModelChange(showModel) }" />
      ...
      <div class="control-row">
        ...
        <label class="switch">
          <input :checked="depthTest" type="checkbox" @change="(e) => { depthTest = (e.target as HTMLInputElement).checked; onDepthTestChange(depthTest) }" />
          <span class="switch-slider"></span>
        </label>
      </div>
      <div class="button-row">
        <button class="action-button primary" @click="resetTransform">重置变换</button>
        <button class="action-button secondary" @click="flyToModel">回到模型</button>
        <button class="action-button secondary" @click="onReset">重新加载</button>
      </div>
      ...
      <div class="control-row">
        ...
        <input v-model.number="axisLength" type="range" min="150" max="8000" step="50" />
        <span class="row-value">{{ axisLength }}</span>
      </div>
      <div class="control-row">
        <span class="row-label">环形轴半径(m)</span>
        <input v-model.number="ringRadius" type="range" min="100" max="5200" step="50" />
      ...
      <p class="hint">操作：大雁塔 3D Tiles 模型已与红黄绿XYZ编辑坐标轴绑定——拖动直线轴或轴端手柄沿该轴平移模型；拖动环形轴绕环所在轴方向旋转模型。平移只改模型中心坐标、旋转只改姿态角，二者互不干扰。悬停高亮为黄色、按下加粗，实时同步坐标与姿态、控制台打印操作信息。</p>
    </div>

    <div v-if="loadingModel" class="status-mask">正在加载大雁塔 3D Tiles 模型…</div>
    <div v-else-if="statusMessage" class="status-mask">{{ statusMessage }}</div>
  </div>
</template>

<style scoped>
.dayanta-xyz-shell { position: relative; width: 100%; height: 100%; min-height: 320px; overflow: hidden; border-radius: 8px; background: #152b4c; }
