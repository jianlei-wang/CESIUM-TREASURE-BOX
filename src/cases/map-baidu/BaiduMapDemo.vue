...
import { BaiduImageryProvider, type BaiduStyle } from '../../lib/map-providers'

type Plan = { id: string; label: string; styles: BaiduStyle[]; desc: string }
const PLANS: Plan[] = [
  {
    id: 'vec',
    label: '方案一·矢量地图',
    styles: ['vec'],
    desc: '原样例：百度矢量地图（style=vec，online 矢量瓦片）'
  },
  {
    id: 'img',
    label: '方案二·卫星影像',
    styles: ['img'],
    desc: '新增方案：百度卫星影像（shangetu 影像瓦片）'
  }
]
...
function apply(): void {
  ...
  const providers = plan.styles.map(
    (style) => new BaiduImageryProvider({ style, crs: crs.value })
  )
...
<template>
  <div class="map-shell">
    ...
    <div class="control-panel">
      <div class="panel-title">百度地图底图</div>
      <div class="hint">百度瓦片服务，自动换算百度瓦片行列号（坐标原点在地图中心）。</div>
