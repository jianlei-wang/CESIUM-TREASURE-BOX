...
const colorHex = ref('#ff4455')
const opacity = ref(0.8)
const radius = ref(300)
const height = ref(300)
const speed = ref(10)

const WALL_ID = 'diffuse-wall'
...
function buildOptions(): Parameters<typeof createDiffuseWall>[1] {
  return {
    id: WALL_ID,
...
function place(lon: number, lat: number): void {
  ...
  CENTER.lon = lon
  CENTER.lat = lat
  createDiffuseWall(viewer, buildOptions())
  resultMessage.value = `扩散墙已放置至 ${lon.toFixed(4)}, ${lat.toFixed(4)}`
}

function updateEffect(): void {
  if (!viewer || viewer.isDestroyed()) return
  updateDiffuseWall(viewer, WALL_ID, {
    radius: radius.value,
    height: height.value,
    speed: speed.value,
    color: Color.fromCssColorString(colorHex.value).withAlpha(opacity.value)
  })
  resultMessage.value = '扩散墙参数已更新'
}
...
watch([colorHex, opacity, radius, height, speed], () => {
...
onMounted(() => {
  ...
  try {
    ...
    viewer.camera.setView({
      destination: Cartesian3.fromDegrees(BASE_CENTER.lon, BASE_CENTER.lat, 100000)
    })
...
onBeforeUnmount(() => {
  handler?.destroy()
  handler = undefined
  removeDiffuseWall(viewer, WALL_ID)
  destroyScene(viewer)
  viewer = undefined
})
</script>
...
<template>
  <div class="diffuse-wall-shell">
    ...
    <div class="control-panel">
      ...
      <div class="control-row">
        <span class="row-label">扩散半径</span>
        <input v-model.number="radius" type="range" min="50" max="1000" step="10" />
        <span class="row-value">{{ radius }}</span>
      </div>
      <div class="control-row">
        <span class="row-label">墙高</span>
        <input v-model.number="height" type="range" min="50" max="800" step="10" />
        <span class="row-value">{{ height }}</span>
      </div>
      <div class="control-row">
        <span class="row-label">扩散速度</span>
        <input v-model.number="speed" type="range" min="1" max="40" step="1" />
        <span class="row-value">{{ speed }}</span>
      </div>

      <p class="hint">环形墙体半径从中心向外扩散、高度随之变化，循环往复；开启放置后单击地图可重新定位。</p>
...
<style scoped>
.diffuse-wall-shell { position: relative; width: 100%; height: 100%; min-height: 320px; overflow: hidden; border-radius: 8px; background: #152b4c; }
