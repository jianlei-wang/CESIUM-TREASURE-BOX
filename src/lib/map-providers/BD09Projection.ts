...
function getRange(v: number, a: number | null, b: number | null): number {
  if (a != null) {
    v = Math.max(v, a)
  }
  if (b != null) {
...
function getLoop(v: number, a: number, b: number): number {
  while (v > b) {
    v -= b - a
  }
  while (v < a) {
...
function convertor(fromPoint: LngLat, factor: number[]): LngLat {
  if (!fromPoint || !factor) {
    return { lng: 0, lat: 0 }
  }
  const x = factor[0] + factor[1] * Math.abs(fromPoint.lng)
  ...
  const lngOut = x * (fromPoint.lng < 0 ? -1 : 1)
  const latOut = y * (fromPoint.lat < 0 ? -1 : 1)
  return { lng: lngOut, lat: latOut }
}

/**
 * 百度墨卡托投影（MC 平面直角坐标 <-> 经纬度）。
 */
class BD09Projection {
  isWgs84 = false

  getDistanceByLL(point1: LngLat, point2: LngLat): number {
  ...
  convertMC2LL(point: Point): LngLat {
    if (!point) {
      return { lng: 0, lat: 0 }
    }
    if (this.isWgs84) {
    ...
    const temp = { lng: Math.abs(point.x), lat: Math.abs(point.y) }
    ...
    const lnglat = convertor(temp, factor as number[])
    return { lng: parseFloat(lnglat.lng.toFixed(6)), lat: parseFloat(lnglat.lat.toFixed(6)) }
  }

  convertLL2MC(point: LngLat): LngLat {
    if (!point) {
      return { lng: 0, lat: 0 }
    }
    ...
    if (this.isWgs84) {
      ...
      const lng = ((point.lng * Math.PI) / 180) * earthRad
      const a = (point.lat * Math.PI) / 180
      const lat = (earthRad / 2) * Math.log((1.0 + Math.sin(a)) / (1.0 - Math.sin(a)))
      return {
    ...
    const mc = convertor(point, factor as number[])
    return {
