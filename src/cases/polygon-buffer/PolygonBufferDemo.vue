...
const steps = ref(32)
const fillColor = ref('#52de9f')
const fillAlpha = ref(0.45)
const showBorder = ref(true)
const borderColor = ref('#1fa06a')
...
const polygonBufferItems: PolygonBufferItem[] = []
...
function applyBufferStyle(item: PolygonBufferItem): void {
...
function rebuildBuffer(item: PolygonBufferItem): void {
  const bufferPoly = polygonBuffer(item.positions.map(toLngLat), makeParams())
...
function createPolygon(vertices: Cartesian3[]): void {
  if (!viewer || viewer.isDestroyed()) return
  viewer.entities.add({
    polygon: {
      hierarchy: new PolygonHierarchy(vertices),
      material: Color.fromCssColorString('#ffd166').withAlpha(0.35)
    }
  })
  viewer.entities.add({
    polyline: {
  ...
  const bufferPoly = polygonBuffer(vertices.map(toLngLat), makeParams())
  if (!bufferPoly) {
  ...
  polygonBufferItems.push({
    positions: vertices,
    bufferPolygon: (bufferEntity as unknown as { polygon: PolygonBufferItem['bufferPolygon'] }).polygon,
    borderPolyline: (borderEntity as unknown as { polyline: PolygonBufferItem['borderPolyline'] }).polyline
  })
  polygonCount.value += 1
  resultMessage.value = `多边形 #${polygonCount.value} 完成（${vertices.length} 个顶点），缓冲区 ${bufferRadius.value}m`
}
...
function updatePreview(): void {
  ...
  if (currentVertices.length >= 2) {
    const borderPositions = lastPreviewPos ? [...currentVertices, lastPreviewPos, currentVertices[0]] : [...currentVertices, currentVertices[0]]
...
function updateExisting(): void {
  if (!viewer || viewer.isDestroyed()) return
  for (const item of polygonBufferItems) {
    rebuildBuffer(item)
...
<template>
  <div class="gbb-shell">
    ...
    <div class="control-panel">
      <div class="panel-title">面缓冲区分析</div>
      ...
      <div class="hint">单击加点，右键或双击闭合；已绘多边形面的缓冲区随参数实时更新。</div>
      <div class="hint">已生成 {{ polygonCount }} 个面缓冲区</div>
...
<style scoped>
.gbb-shell { position: relative; width: 100%; height: 100%; min-height: 320px; overflow: hidden; border-radius: 8px; background: #152b4c; }
