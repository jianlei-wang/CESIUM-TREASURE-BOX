...
import { LandslideCesiumScene, STAGES } from './LandslideCesiumScene'
...
let scene: LandslideCesiumScene | null = null
...
const stageTxt = [
  '坡体缓慢蠕滑，后缘拉张裂缝张开、地面下错，坡脚渗水，树木歪斜。此阶段可持续数天至数年，是最宝贵的预警窗口期。',
  '裂缝贯通、滑带抗剪强度被克服，滑体剪出后整体启动并迅速加速，坡体质心明显下移。',
  '滑体沿滑动面高速下滑，前端受阻破碎、解体为碎屑流，能量释放达到峰值。',
  '滑体推挤堆积于坡脚，形成滑坡舌与鼓张裂缝，运动逐渐停止并趋于新的稳定状态。'
]
...
onMounted(() => {
  ...
  scene = new LandslideCesiumScene(canvasHost.value, { onProgress })
})
...
const legend = [
  { color: '#5f8a3e', label: '稳定山体 / 植被坡面' },
  { color: '#8a6a42', label: '滑坡体（滑动岩土体）' },
  { color: '#e0b64a', label: '滑动面 / 滑带' },
  { color: '#9c7c42', label: '滑坡床（裸露滑面）' },
...
<template>
  <div class="fy-shell">
    <div ref="canvasHost" class="cesium-container"></div>

    <div class="control-panel">
      <div class="panel-title-row">
        <span class="panel-title">滑坡形成与运动演示</span>
