...
import { TencentImageryProvider, type TencentStyle } from '../../lib/map-providers'

type Plan = { id: string; label: string; styles: TencentStyle[]; desc: string }
const PLANS: Plan[] = [
  {
    id: 'elec',
    label: '方案一·电子地图',
    styles: ['elec'],
    desc: '原样例：腾讯电子地图（styleid=1 经典样式）'
  },
  {
    id: 'img',
    label: '方案二·卫星影像',
    styles: ['img'],
    desc: '新增方案：腾讯卫星影像（customTags 计算 {sx}/{sy} 分块）'
  }
]
...
function apply(): void {
  ...
  const providers = plan.styles.map(
    (style) => new TencentImageryProvider({ style, crs: crs.value })
  )
...
<template>
  <div class="map-shell">
    ...
    <div class="control-panel">
      <div class="panel-title">腾讯地图底图</div>
      <div class="hint">腾讯瓦片服务，影像瓦片动态计算分块号。</div>
