...
import { CIRCLE_FADE_TYPE, registerDcEffectsMaterials } from '../dc-effects-lib'
...
function place(lon: number, lat: number): void {
  ...
  removeDcEffectEntity(viewer, CIRCLE_FADE_TYPE)
  createDcEffectEntity(viewer, {
    type: CIRCLE_FADE_TYPE,
    lon,
    lat,
    height: height.value,
    radius: radius.value,
  ...
  resultMessage.value = `消逝圆已放置于 ${lon.toFixed(4)}, ${lat.toFixed(4)}`
}

function updateEffect(): void {
  if (!viewer || viewer.isDestroyed()) return
  const entity = viewer.entities.getById(`dc-effect-${CIRCLE_FADE_TYPE}`)
  if (!entity) {
    place(DEFAULT_LON, DEFAULT_LAT)
    return
  }
  updateDcEffectEntity(viewer, CIRCLE_FADE_TYPE, {
    radius: radius.value,
    height: height.value,
    color: Color.fromCssColorString(colorHex.value),
    speed: speed.value
  })
  resultMessage.value = '消逝圆参数已更新'
}
...
<style scoped>
.fade-shell { position: relative; width: 100%; height: 100%; min-height: 320px; overflow: hidden; border-radius: 8px; background: #152b4c; }
