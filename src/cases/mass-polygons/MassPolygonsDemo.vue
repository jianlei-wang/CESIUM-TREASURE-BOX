...
const span = ref(1)
const vertexMode = ref<'triangle' | 'quad'>('triangle')
...
function randomPositions(): number[] {
  const vertexCount = vertexMode.value === 'triangle' ? 3 : 4
...
function removePrimitive(): void {
  if (polygonPrimitive && viewer && !viewer.isDestroyed()) {
      ...
      instances.push(
        new GeometryInstance({
          geometry: new PolygonGeometry({
            polygonHierarchy: new PolygonHierarchy(Cartesian3.fromDegreesArray(randomPositions())),
            height: height.value,
...
<template>
  <div class="mass-shell">
    ...
    <div class="control-panel">
      <div class="panel-title">海量不规则多边形加载</div>
      ...
      <div class="control-row">
        <span class="row-label">形状</span>
        <div class="mode-group">
          <button class="mode-button" :class="{ active: vertexMode === 'triangle' }" :disabled="generating" @click="vertexMode = 'triangle'">三角形</button>
          <button class="mode-button" :class="{ active: vertexMode === 'quad' }" :disabled="generating" @click="vertexMode = 'quad'">四边形</button>
        </div>
      </div>
      <div class="control-row">
      ...
      <div class="control-row">
        ...
        <input v-model.number="height" type="range" min="0" max="1000" step="50" :disabled="generating" />
      ...
      <p class="hint">
        <template v-if="generating">正在生成 {{ count >= 10000 ? `${count / 10000}万` : count }} 个不规则多边形，请稍候…</template>
