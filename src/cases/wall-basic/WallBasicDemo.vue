...
function place(lon: number, lat: number): void {
  ...
  resultMessage.value = `基础墙体已放置于 ${lon.toFixed(4)}, ${lat.toFixed(4)}`
}
...
watch([colorHex, opacity, widthMeters, depthMeters, height], () => {
...
onMounted(() => {
  ...
  try {
    ...
    viewer.camera.setView({
      destination: Cartesian3.fromDegrees(DEFAULT_LON, DEFAULT_LAT, 5000)
    })
...
<template>
  <div class="wall-basic-shell">
    <div ref="container" class="cesium-container"></div>

    <div class="control-panel">
      <div class="panel-title">基础墙体效果</div>
      ...
      <div class="control-row">
        <span class="row-label">顶部描边</span>
        <label class="switch-label">
          <input v-model="showOutline" type="checkbox" class="switch-input" />
          <span class="switch-slider"></span>
        </label>
      </div>

      <p class="hint">墙体颜色自上而下渐变发光；开启放置后单击地图可移动位置，参数修改实时生效。</p>
...
<style scoped>
.wall-basic-shell { position: relative; width: 100%; height: 100%; min-height: 320px; overflow: hidden; border-radius: 8px; background: #152b4c; }
