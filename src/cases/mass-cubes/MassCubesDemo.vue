...
const width = ref(40000)
const height = ref(30000)
const depth = ref(500000)
const sizeRandom = ref(true)
const colorMode = ref<'random' | 'single' | 'alternate'>('random')
const color1 = ref('#ff5c5c')
const color2 = ref('#4a9eff')
const opacity = ref(0.25)
const distribution = ref<'china' | 'world'>('world')
...
let cubePrimitive: Primitive | undefined
...
function randomPosition(index: number, total: number): Cartesian3 {
  if (distribution.value === 'world') {
    const longitude = Math.random() * 360 - 180
    const latitude = Math.random() * 180 - 90
    return Cartesian3.fromDegrees(longitude, latitude, 0)
  }
  const longitude = 73 + Math.random() * 62
  const latitude = 18 + Math.random() * 36
  return Cartesian3.fromDegrees(longitude, latitude, 0)
}

function randomDimensions(): Cartesian3 {
  if (!sizeRandom.value) {
    return new Cartesian3(width.value, height.value, depth.value)
  }
...
function resolveColor(index: number): Color {
  ...
  if (colorMode.value === 'single') {
    return Color.fromCssColorString(color1.value).withAlpha(alpha)
  }
  if (colorMode.value === 'random') {
    return Color.fromHsl(Math.random(), 0.8, 0.6, alpha)
  }
...
function removePrimitive(): void {
  if (cubePrimitive && viewer && !viewer.isDestroyed()) {
    viewer.scene.primitives.remove(cubePrimitive)
  }
...
async function regenerate(): Promise<void> {
  ...
  for (let i = 0; i < total; i += chunkSize) {
    ...
    for (let j = i; j < end; j++) {
      const position = randomPosition(j, total)
      const dimensions = randomDimensions()
      instances.push(
...
onMounted(() => {
  ...
  try {
    ...
    viewer.camera.setView({
      destination: Cartesian3.fromDegrees(105, 36, 10000000)
    })
    regenerate()
  } catch (error) {
...
<template>
  <div class="mass-shell">
    ...
    <div class="control-panel">
      <div class="panel-title">海量随机立方体加载</div>
      ...
      <div class="control-row">
        ...
        <input v-model.number="width" type="range" min="5000" max="100000" step="5000" :disabled="generating" />
        <span class="row-value">{{ width }}</span>
      </div>
      <div class="control-row">
        <span class="row-label">高度(m)</span>
        <input v-model.number="height" type="range" min="5000" max="100000" step="5000" :disabled="generating" />
        <span class="row-value">{{ height }}</span>
      </div>
      <div class="control-row">
        <span class="row-label">深度(m)</span>
        <input v-model.number="depth" type="range" min="50000" max="1000000" step="50000" :disabled="generating" />
        <span class="row-value">{{ depth }}</span>
      </div>
      <div class="control-row">
        <span class="row-label">尺寸随机</span>
        <label class="switch">
          <input v-model="sizeRandom" type="checkbox" :disabled="generating" />
      ...
      <div class="control-row">
        ...
        <input v-model.number="opacity" type="range" min="0.05" max="1" step="0.05" :disabled="generating" />
      ...
      <p class="hint">
        <template v-if="generating">正在生成 {{ count >= 10000 ? `${count / 10000}万` : count }} 个立方体，请稍候…</template>
        <template v-else>当前已渲染 {{ renderedCount >= 10000 ? `${renderedCount / 10000}万` : renderedCount }} 个{{ elapsedMs ? `，耗时 ${elapsedMs} ms` : '' }}</template>
      </p>
