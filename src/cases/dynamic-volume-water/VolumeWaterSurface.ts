...
export const WATER_RANGE_MODES = {
  RECTANGLE: 'rectangle',
  POLYGON: 'polygon'
} as const

export type WaterRangeMode = (typeof WATER_RANGE_MODES)[keyof typeof WATER_RANGE_MODES]
...
const DEFAULT_WATER_PARAMS: WaterParams = {
  ...
  polygonPositions: LIJIANG_WATER_POSITIONS,
  deepColor: [8, 66, 96],
...
type WaterUniforms = Record<string, number | Cesium.Cartesian4>
...
export class VolumeWaterSurface {
  viewer: Cesium.Viewer
  params: WaterParams
  primitive: Cesium.Primitive | null = null
  uniforms: WaterUniforms | null = null
  area: WaterArea | null = null
  startTime = performance.now()
  private preRenderListener: () => void

  constructor(options: { viewer: Cesium.Viewer; params?: Partial<WaterParams> }) {
    this.viewer = options.viewer
    this.params = createWaterParams(options.params)
  ...
  rebuild = (): void => {
    ...
    const renderState = Cesium.Appearance.getDefaultRenderState(true, false, {
      depthTest: {
        enabled: true
      }
    ...
    this.primitive = this.viewer.scene.primitives.add(
      new Cesium.Primitive({
        ...
        appearance: new Cesium.Appearance({
          ...
          renderState: renderState as never,
          translucent: true,
          closed: false,
    ...
    if (this.primitive) {
      ;(this.primitive.appearance as unknown as { uniforms: WaterUniforms }).uniforms = this.uniforms
    }
  ...
  updateUniforms = (): void => {
    ...
    if (this.params.animate) {
      time = (performance.now() - this.startTime) * 0.001 * this.params.timeScale
    }
    ...
    this.uniforms.u_vertexTime = time
  ...
  setPolygon(points: PolygonPosition[]): boolean {
    const normalized = normalizePolygonPositions(points)
    if (normalized.length < 3) return false
    this.params.rangeMode = WATER_RANGE_MODES.POLYGON
    this.params.polygonPositions = normalized
    this.rebuild()
    this.flyTo()
    return true
  }

  setRectangle(lon: number, lat: number, width: number, depth: number, height: number): void {
    this.params.rangeMode = WATER_RANGE_MODES.RECTANGLE
    this.params.planeLon = lon
    this.params.planeLat = lat
    this.params.planeWidth = width
    this.params.planeDepth = depth
    this.params.planeHeight = height
    this.rebuild()
    this.flyTo()
  }

  flyTo(duration = 1.5): void {
...
function resolveWaterArea(params: WaterParams): WaterArea {
...
function createRectangleArea(params: WaterParams): WaterArea {
...
function createPolygonArea(params: WaterParams, polygonPositions: PolygonPosition[]): WaterArea {
  const centerLon = polygonPositions.reduce((total, point) => total + point[0], 0) / polygonPositions.length
  const centerLat = polygonPositions.reduce((total, point) => total + point[1], 0) / polygonPositions.length
  const center = createCenterFrame(centerLon, centerLat, params.planeHeight)
  const inverseFrame = Cesium.Matrix4.inverseTransformation(center.modelMatrix, new Cesium.Matrix4())
  const localPolygon = polygonPositions.map((point) => {
    const world = Cesium.Cartesian3.fromDegrees(point[0], point[1], params.planeHeight)
    const local = Cesium.Matrix4.multiplyByPoint(inverseFrame, world, new Cesium.Cartesian3())
  ...
  return {
    ...
    containsPoint: (x: number, y: number) => isPointInPolygon(x, y, localPolygon)
  }
}

function createCenterFrame(lon: number, lat: number, height: number): { modelMatrix: Cesium.Matrix4 } {
...
function normalizePolygonPositions(positions: PolygonPosition[] | undefined | null): PolygonPosition[] {
  ...
  const normalized = positions
    .map((position) => {
      ...
      const point = position as { lon?: number; lat?: number; longitude?: number; latitude?: number }
      return {
  ...
  return normalized.map((position) => [position.lon, position.lat])
}

function getLocalBounds(points: LocalPoint[]): { minX: number; maxX: number; minY: number; maxY: number } {
...
function isPointInPolygon(x: number, y: number, polygon: LocalPoint[]): boolean {
  let inside = false
...
function createWaterMeshGeometry(params: WaterParams, area: WaterArea): Cesium.Geometry {
  ...
  const bitangentValues: number[] = []
  const batchIdValues: number[] = []
  ...
  for (let y = 0; y < ySegments; y++) {
    for (let x = 0; x < xSegments; x++) {
      ...
      indices.push(i0 as number, i2 as number, i1 as number, i1 as number, i2 as number, i3 as number)
    }
  }
...
function shouldKeepPolygonVertex(x: number, y: number, xSegments: number, ySegments: number, area: WaterArea): boolean {
...
function createGeometryFromArrays(options: {
  positions: Float64Array
  st: Float32Array
  normals: Float32Array
  tangents: Float32Array
  bitangents: Float32Array
  batchIds: Float32Array
  ...
  const geometryOptions = {
    attributes: {
      ...
      batchId: new Cesium.GeometryAttribute({
        ...
        values: options.batchIds
      })
    },
    indices: options.indices,
    primitiveType: Cesium.PrimitiveType.TRIANGLES,
    boundingSphere: new Cesium.BoundingSphere(Cesium.Cartesian3.ZERO, options.boundingRadius)
  } as unknown as ConstructorParameters<typeof Cesium.Geometry>[0]

  return new Cesium.Geometry(geometryOptions)
}

function createTypedIndices(indices: number[], vertexCount: number): Uint16Array | Uint32Array {
...
function createWaterParams(overrides?: Partial<WaterParams>): WaterParams {
  return {
    ...
    polygonPositions: DEFAULT_WATER_PARAMS.polygonPositions.map((position) => [...position]),
    ...overrides
  }
}

function createWaterUniforms(): WaterUniforms {
  return {
    u_time: 0,
...
function colorArrayToVec4(value: [number, number, number], alpha: number): Cesium.Cartesian4 {
  return new Cesium.Cartesian4(value[0] / 255, value[1] / 255, value[2] / 255, alpha)
}
