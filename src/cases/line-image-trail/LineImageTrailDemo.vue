...
const speed = ref(3)
const repeatX = ref(8)
const lineWidth = ref(12)

const LINE_ID = 'line-image-trail'
...
function place(lon: number, lat: number): void {
  ...
  resultMessage.value = `图片轨迹线已平移至 ${lon.toFixed(4)}, ${lat.toFixed(4)}`
}
...
function updateEffect(): void {
  ...
  resultMessage.value = '图片轨迹线参数已更新'
}
...
watch([colorHex, opacity, speed, repeatX, lineWidth], () => {
...
<template>
  <div class="line-image-trail-shell">
    <div ref="container" class="cesium-container"></div>

    <div class="control-panel">
      <div class="panel-title">图片轨迹线效果</div>
      ...
      <p class="hint">箭头图片沿路径水平流动形成轨迹；开启放置后单击地图可整体平移折线，参数修改实时生效。</p>
...
<style scoped>
.line-image-trail-shell { position: relative; width: 100%; height: 100%; min-height: 320px; overflow: hidden; border-radius: 8px; background: #152b4c; }
