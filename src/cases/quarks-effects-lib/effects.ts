...
function bool(values: ParamValues, key: string, fallback: boolean): boolean {
  const value = values[key]
...
function alphaFade(colorStops: Array<[number, number]>, opacity: number): Gradient {
  ...
  return new Gradient(
    ...
    colorStops.map(([a, t]) => [a * opacity, t] as [number, number])
  )
}
...
function buildFountain(values: ParamValues, ctx: EffectBuildContext): BuiltEffect {
  const rate = num(values, 'rate', 200)
  const life = num(values, 'life', 2.4)
  const speed = num(values, 'speed', 9)
  const size = num(values, 'size', 0.55)
  ...
  const jet = new ParticleSystem({
    ...
    looping: true,
    prewarm: true,
    shape: new ConeEmitter({ radius, angle: spread * DEG, thickness: 0.6 }),
    startLife: new IntervalValue(life * 0.6, life),
    startSpeed: new IntervalValue(speed * 0.85, speed * 1.15),
    startSize: new IntervalValue(size * 0.7, size),
    startColor: new ColorRange(hexToQ4('#eaf7ff'), hexToQ4('#8fd0ff')),
    emissionOverTime: new ConstantValue(rate * 0.75),
    behaviors: [
      ...
      ...(turbulence > 0
        ? [new TurbulenceField(q3(2.5, 2.5, 2.5), 2, q3(turbulence, turbulence, turbulence), q3(0.6, 0.6, 0.6))]
        : [])
    ],
  ...
  const spray = new ParticleSystem({
    ...
    looping: true,
    prewarm: true,
    shape: new ConeEmitter({ radius: radius * 1.4, angle: spread * 1.9 * DEG, thickness: 0.9 }),
    startLife: new IntervalValue(life * 0.45, life * 0.8),
    startSpeed: new IntervalValue(speed * 0.25, speed * 0.6),
    startSize: new IntervalValue(size * 1.4, size * 2.6),
    startColor: new ColorRange(hexToQ4('#ffffff'), hexToQ4('#bfe3ff')),
    emissionOverTime: new ConstantValue(rate * 0.5 * mist),
    behaviors: [
      new ApplyForce(q3(0, -1, 0), new ConstantValue(gravity * 0.35)),
      new ApplyForce(q3(0, 0, 0), new ConstantValue(0)),
      ...
      ...(turbulence > 0
        ? [new TurbulenceField(q3(3, 3, 3), 2, q3(turbulence * 0.6, turbulence * 0.6, turbulence * 0.6), q3(0.4, 0.4, 0.4))]
        : [])
    ],
...
function buildFlame(values: ParamValues, ctx: EffectBuildContext): BuiltEffect {
  ...
  const flame = new ParticleSystem({
    ...
    looping: true,
    prewarm: true,
    shape: new ConeEmitter({ radius, angle: spread * DEG, thickness: 0.85 }),
    startLife: new IntervalValue(life * 0.55, life),
    startSpeed: new IntervalValue(speed * 0.6, speed * 1.2),
    startSize: new IntervalValue(size * 0.7, size * 1.2),
    startColor: new ColorRange(hexToQ4('#ffffff'), hexToQ4('#ffffff')),
    emissionOverTime: new ConstantValue(rate),
    behaviors: [
      new ApplyForce(q3(0, 1, 0), new ConstantValue(buoyancy)),
      new SizeOverLife(new PiecewiseBezier([[new Bezier(0.05, 1.3, 0.55, 0), 0]])),
      new ColorOverLife(
        gradientFromHex(
          [
            ['#fff6c8', 0],
      ...
      ...(turbulence > 0
        ? [new TurbulenceField(q3(2, 2, 2), 2, q3(turbulence, turbulence * 1.4, turbulence), q3(1.2, 1.2, 1.2))]
        : [])
    ],
    renderMode: RenderMode.BillBoard,
    material: additive(ctx.textures.glow),
  ...
  const sparks = new ParticleSystem({
    ...
    looping: true,
    prewarm: true,
    shape: new ConeEmitter({ radius: radius * 0.7, angle: spread * 1.4 * DEG, thickness: 0.5 }),
    startLife: new IntervalValue(life * 0.35, life * 0.7),
    startSpeed: new IntervalValue(speed * 1.6, speed * 3),
    startSize: new IntervalValue(size * 0.1, size * 0.24),
    startColor: new ColorRange(hexToQ4('#ffffff'), hexToQ4('#ffffff')),
    emissionOverTime: new ConstantValue(rate * 0.35),
    behaviors: [
      new ApplyForce(q3(0, 1, 0), new ConstantValue(buoyancy * 1.6)),
      ...
      ...(turbulence > 0
        ? [new TurbulenceField(q3(3, 3, 3), 2, q3(turbulence * 1.4, turbulence * 1.6, turbulence * 1.4), q3(1.6, 1.6, 1.6))]
        : [])
    ],
    renderMode: RenderMode.StretchedBillBoard,
    speedFactor: 1,
...
function buildSmoke(values: ParamValues, ctx: EffectBuildContext): BuiltEffect {
  ...
  const size = num(values, 'size', 1.6)
  ...
  const smoke = new ParticleSystem({
    ...
    startLife: new IntervalValue(life * 0.65, life),
    startSpeed: new IntervalValue(speed * 0.5, speed * 1.3),
    startSize: new IntervalValue(size * 0.55, size),
    startColor: new ColorRange(hexToQ4('#3c3f46'), hexToQ4('#8d9299')),
    emissionOverTime: new ConstantValue(rate),
    behaviors: [
      new ApplyForce(q3(0, 1, 0), new ConstantValue(rise)),
      new ApplyForce(q3(0.6, 0, 0.85), new ConstantValue(wind)),
      ...
      ...(turbulence > 0
        ? [new TurbulenceField(q3(3.5, 3.5, 3.5), 2, q3(turbulence, turbulence * 0.7, turbulence), q3(0.25, 0.25, 0.25))]
        : [])
    ],
    renderMode: RenderMode.BillBoard,
    material: normalBlend(ctx.textures.smoke),
    rendererEmitterSettings: {}
  })
  smoke.emitter.rotation.x = -Math.PI / 2

  return { systems: [smoke] }
}
...
function buildFireworks(values: ParamValues, ctx: EffectBuildContext): BuiltEffect {
  const count = num(values, 'count', 220)
  const interval = num(values, 'interval', 1.8)
  const speed = num(values, 'speed', 22)
  const life = num(values, 'life', 2.4)
  const size = num(values, 'size', 0.5)
  const gravity = num(values, 'gravity', 6)
  const sphereRadius = num(values, 'radius', 1.1)
  ...
  const shell = new ParticleSystem({
    ...
    autoDestroy: false,
    prewarm: false,
    shape: new SphereEmitter({ radius: sphereRadius, thickness: 1 }),
    startLife: new IntervalValue(life * 0.7, life),
    startSpeed: new IntervalValue(speed * 0.55, speed),
    startSize: new IntervalValue(size * 0.5, size),
    startColor: new ColorRange(colorA, colorB),
    emissionOverTime: new ConstantValue(0),
    emissionBursts: [{ time: 0, count: new ConstantValue(Math.round(count * 0.7)), cycle: 1, interval: 0, probability: 1 }],
    behaviors: [
      new ApplyForce(q3(0, -1, 0), new ConstantValue(gravity)),
  ...
  const sparks = new ParticleSystem({
    ...
    prewarm: false,
    shape: new SphereEmitter({ radius: sphereRadius, thickness: 1 }),
    startLife: new IntervalValue(life * 0.85, life * 1.25),
    startSpeed: new IntervalValue(speed * 0.8, speed * 1.25),
    startSize: new IntervalValue(size * 0.16, size * 0.34),
    startColor: new ColorRange(colorA, hexToQ4('#ffffff')),
    emissionOverTime: new ConstantValue(0),
    emissionBursts: [
      { time: 0, count: new ConstantValue(Math.round(count * 0.5)), cycle: 1, interval: 0, probability: 1 }
    ],
    behaviors: [
      new ApplyForce(q3(0, -1, 0), new ConstantValue(gravity * 1.1)),
...
export const EFFECT_META: Record<EffectId, EffectMeta> = {
  fountain: {
    ...
    subtitle: 'GPU 粒子水景',
    description: '基于 three.quarks 的 GPU 粒子喷泉，叠加在 Cesium 地球上，实时呈现水流、水雾与重力回落。',
    params: [
      { key: 'rate', label: '发射速率', kind: 'number', min: 20, max: 600, step: 10, unit: '个/秒', default: 200 },
      { key: 'life', label: '粒子寿命', kind: 'number', min: 0.5, max: 6, step: 0.1, unit: '秒', default: 2.4 },
      { key: 'speed', label: '喷射速度', kind: 'number', min: 1, max: 25, step: 0.5, default: 9 },
      { key: 'size', label: '粒子尺寸', kind: 'number', min: 0.05, max: 2, step: 0.05, default: 0.55 },
      { key: 'gravity', label: '重力加速度', kind: 'number', min: 0, max: 25, step: 0.5, default: 9.8 },
      { key: 'spread', label: '喷射张角', kind: 'number', min: 0, max: 45, step: 1, unit: '°', default: 14 },
      ...
      { key: 'mist', label: '水雾浓度', kind: 'number', min: 0, max: 2, step: 0.1, default: 1 }
    ]
  },
  flame: {
    id: 'flame',
    ...
    subtitle: 'GPU 粒子火焰',
    description: '基于 three.quarks 的 GPU 粒子火焰，包含明火主体与飞溅火星，叠加在 Cesium 地球上实时燃烧。',
    params: [
      { key: 'rate', label: '发射速率', kind: 'number', min: 40, max: 900, step: 10, unit: '个/秒', default: 320 },
      { key: 'life', label: '粒子寿命', kind: 'number', min: 0.4, max: 4, step: 0.1, unit: '秒', default: 1.6 },
      { key: 'speed', label: '初始速度', kind: 'number', min: 0.5, max: 12, step: 0.1, default: 3.2 },
      { key: 'size', label: '粒子尺寸', kind: 'number', min: 0.1, max: 3, step: 0.05, default: 0.9 },
      { key: 'buoyancy', label: '浮力强度', kind: 'number', min: 0, max: 14, step: 0.2, default: 4.2 },
      { key: 'spread', label: '火焰张角', kind: 'number', min: 0, max: 45, step: 1, unit: '°', default: 16 },
      { key: 'radius', label: '火源半径', kind: 'number', min: 0, max: 2, step: 0.05, default: 0.3 },
  ...
  smoke: {
    ...
    subtitle: 'GPU 粒子烟雾',
    description: '基于 three.quarks 的 GPU 粒子烟雾，通过湍流场与风向力驱动烟雾上升扩散，叠加在 Cesium 地球上。',
    params: [
      { key: 'rate', label: '发射速率', kind: 'number', min: 5, max: 240, step: 5, unit: '个/秒', default: 60 },
      { key: 'life', label: '粒子寿命', kind: 'number', min: 1, max: 14, step: 0.5, unit: '秒', default: 6 },
      { key: 'speed', label: '初始速度', kind: 'number', min: 0.1, max: 6, step: 0.1, default: 1.1 },
      { key: 'size', label: '粒子尺寸', kind: 'number', min: 0.3, max: 6, step: 0.1, default: 1.6 },
      { key: 'rise', label: '上升力', kind: 'number', min: 0, max: 8, step: 0.1, default: 1.5 },
      { key: 'wind', label: '风力', kind: 'number', min: 0, max: 5, step: 0.1, default: 0.6 },
  ...
  fireworks: {
    ...
    subtitle: 'GPU 粒子烟花',
    description: '基于 three.quarks 的 GPU 粒子烟花，多层爆裂球壳与拖尾火星在 Cesium 地球上空随机绽放。',
    params: [
      { key: 'count', label: '爆裂粒子数', kind: 'number', min: 40, max: 600, step: 10, unit: '个', default: 220 },
      { key: 'interval', label: '发射间隔', kind: 'number', min: 0.4, max: 6, step: 0.1, unit: '秒', default: 1.8 },
      { key: 'speed', label: '爆裂速度', kind: 'number', min: 5, max: 50, step: 1, default: 22 },
      { key: 'life', label: '粒子寿命', kind: 'number', min: 0.6, max: 5, step: 0.1, unit: '秒', default: 2.4 },
      { key: 'size', label: '粒子尺寸', kind: 'number', min: 0.1, max: 2, step: 0.05, default: 0.5 },
      { key: 'gravity', label: '重力加速度', kind: 'number', min: 0, max: 25, step: 0.5, default: 6 },
      { key: 'radius', label: '爆裂半径', kind: 'number', min: 0, max: 4, step: 0.1, default: 1.1 },
      {
        key: 'palette',
        label: '配色方案',
        kind: 'select',
