...
import { RAIN_FRAGMENT } from '../../lib/weather'
...
function updateStage(): void {
  ...
  stage.uniforms.len = rainLength.value
  stage.uniforms.brightness = brightness.value
}

watch([enabled, density, speed, wind, rainLength, brightness], updateStage)
...
<template>
  <div class="weather-shell">
    ...
    <div class="weather-panel">
      <div class="panel-title">下雨效果</div>
      <div class="toggle-row">
        <span class="toggle-label">效果开关</span>
        <button class="toggle" :class="{ on: enabled }" :aria-label="enabled ? '关闭下雨' : '开启下雨'" @click="enabled = !enabled">
          <i></i>
        </button>
      </div>
      <div class="param-row">
        <span class="param-label">雨量</span>
      ...
      <div class="param-row">
        <span class="param-label">雨丝</span>
        <input v-model.number="rainLength" class="param-slider" type="range" min="0.1" max="1" step="0.05" />
        <span class="param-value">{{ rainLength.toFixed(2) }}</span>
      </div>
      <div class="param-row">
