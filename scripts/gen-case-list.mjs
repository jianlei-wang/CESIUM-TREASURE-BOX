 ...
 * - 创建时间优先取 updatedAt，缺失时回退到案例目录 / index.ts 的文件系统时间（birthtime，其次 mtime）；
 * - 文章输出为人工维护列：重新生成时会按案例名称保留现有 ✅/❌ 状态，新案例默认 ❌；
...
function readExistingStatuses() {
  const statuses = new Map()
  if (!existsSync(listPath)) return statuses
  const source = readFileSync(listPath, 'utf8')
  for (const line of source.split('\n')) {
    const m = line.match(/^\|\s*\d+\s*\|\s*(.+?)\s*\|\s*[^|]*\|\s*([^|]*?)\s*\|\s*$/)
    if (!m) continue
    const name = m[1].trim()
    const flag = m[2].replace(/\uFE0F/g, '').trim()
    if (flag.startsWith(DONE.replace(/\uFE0F/g, ''))) statuses.set(name, true)
    else if (flag.startsWith(TODO.replace(/\uFE0F/g, ''))) statuses.set(name, false)
  }
  return statuses
}
...
const cases = dirs.map((dir) => {
  ...
  const title = propOf(source, 'title') || dir
  const created = propOf(source, 'updatedAt') || fallbackDate(dir)
  return { dir, title: title.replace(/\|/g, '/'), created }
})
...
const previous = readExistingStatuses()
const doneCount = cases.filter((c) => previous.get(c.title)).length
...
lines.push('')
lines.push('| 序号 | 案例名称 | 创建时间 | 文章输出 |')
lines.push('| --- | --- | --- | --- |')
cases.forEach((c, i) => {
  const flag = previous.get(c.title) ? DONE : TODO
  lines.push(`| ${i + 1} | ${c.title} | ${c.created || '-'} | ${flag} |`)
})
lines.push('')
