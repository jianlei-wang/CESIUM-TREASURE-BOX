...
const EXPLOSION_COLORS = ['#ffffff', '#fff4b0', '#ffd400', '#ff8c00', '#ff3d00', '#8a1500']
...
function applyOptions(): void {
  ...
  layer.updateOptions({
    size: controls.size,
    initialSpeed: [controls.power * 0.5, controls.power],
...
function flyToEmitter(): void {
  ...
  viewer.camera.flyTo({
    destination: Cartesian3.fromDegrees(emitter.lon, emitter.lat, emitter.height + 1500)
  })
}
...
<template>
  <div class="effect-shell">
    ...
    <div class="effect-panel">
      <div class="panel-title">GPU 爆炸粒子</div>

      <div class="section">
        <div class="section-title">爆点（点击地图引爆）</div>
        <div class="grid-row">
          <label class="field">
            <span>经度</span>
        ...
        <div class="btn-row">
          <button class="file-btn" @click="flyToEmitter">飞往爆点</button>
          <button class="file-btn primary" @click="triggerExplosion">重新引爆</button>
        </div>
      </div>
      ...
      <div class="section">
        ...
        <label class="slider-row">
          <span>爆炸威力 {{ controls.power }}</span>
          <input v-model.number="controls.power" type="range" min="15" max="120" step="1" @input="applyOptions" />
        </label>
        <label class="slider-row">
          <span>寿命 {{ controls.lifetime.toFixed(1) }}s</span>
          <input v-model.number="controls.lifetime" type="range" min="0.8" max="4" step="0.1" @input="applyOptions" />
        </label>
        <label class="slider-row">
          <span>重力 {{ controls.gravity }}</span>
          <input v-model.number="controls.gravity" type="range" min="0" max="30" step="1" @input="applyOptions" />
        </label>
        <label class="slider-row">
          <span>湍流 {{ controls.turbulence }}</span>
          <input v-model.number="controls.turbulence" type="range" min="0" max="20" step="1" @input="applyOptions" />
        </label>
        <label class="slider-row">
          <span>爆炸半径 {{ controls.radius }}m</span>
          <input v-model.number="controls.radius" type="range" min="0" max="10" step="0.5" @input="applyOptions" />
        </label>
