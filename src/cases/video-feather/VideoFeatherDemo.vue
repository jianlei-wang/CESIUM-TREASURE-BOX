...
const widthMeters = ref(1600)
const heightMeters = ref(1100)
const opacity = ref(1)
const featherWidth = ref(0.3)
const playing = ref(true)

const DEFAULT_LON = 120.38
const DEFAULT_LAT = 36.08

const VIDEO_ID = 'video-feather'
...
function buildOptions(): VideoPrimitiveOptions {
  return {
    ...
    materialKind: 'VideoFeather',
    opacity: opacity.value,
    featherWidth: featherWidth.value
  }
}
...
function place(lon: number, lat: number): void {
  ...
  resultMessage.value = `羽化视频已放置于 ${lon.toFixed(4)}, ${lat.toFixed(4)}`
}
...
watch([widthMeters, heightMeters], () => {
  if (viewer && !viewer.isDestroyed() && video) {
    moveVideoPrimitive(viewer, VIDEO_ID, buildOptions())
    resultMessage.value = '羽化视频参数已更新'
  }
})

watch([opacity, featherWidth], () => {
  if (viewer && !viewer.isDestroyed() && video) {
...
<template>
  <div class="video-feather-shell">
    ...
    <div class="control-panel">
      ...
      <div class="control-row">
        <span class="row-label">羽化宽度</span>
        <input v-model.number="featherWidth" type="range" min="0.05" max="0.5" step="0.05" />
        <span class="row-value">{{ featherWidth.toFixed(2) }}</span>
      </div>
      <div class="control-row">
      ...
      <p class="hint">视频四周经 alpha 羽化渐隐，与地形柔和过渡；羽化宽度越大，过渡带越宽。</p>
...
<style scoped>
.video-feather-shell { position: relative; width: 100%; height: 100%; min-height: 320px; overflow: hidden; border-radius: 8px; background: #152b4c; }
