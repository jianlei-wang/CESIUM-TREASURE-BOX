...
function place(lon: number, lat: number): void {
  ...
  resultMessage.value = `视频融合已放置于 ${lon.toFixed(4)}, ${lat.toFixed(4)}`
}
...
<template>
  <div class="video-fusion-shell">
    ...
    <div class="control-panel">
      ...
      <div class="control-row">
        <span class="row-label">融合透明度</span>
        <input v-model.number="opacity" type="range" min="0.1" max="0.95" step="0.05" />
      ...
      <p class="hint">视频半透明叠加于地形，地形透出形成影像融合效果；透明度越低，地形越清晰。</p>
...
<style scoped>
.video-fusion-shell { position: relative; width: 100%; height: 100%; min-height: 320px; overflow: hidden; border-radius: 8px; background: #152b4c; }
