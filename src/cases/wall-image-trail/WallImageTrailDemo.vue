...
const speed = ref(6)
const repeatX = ref(12)
const widthMeters = ref(1000)
...
const WALL_ID = 'wall-image-trail'
...
function place(lon: number, lat: number): void {
  ...
  resultMessage.value = `流动图片墙体已放置于 ${lon.toFixed(4)}, ${lat.toFixed(4)}`
}
...
function updateEffect(): void {
  ...
  resultMessage.value = '流动图片墙体参数已更新'
}
...
watch([colorHex, opacity, speed, repeatX, widthMeters, depthMeters, height], () => {
...
<template>
  <div class="wall-image-trail-shell">
    <div ref="container" class="cesium-container"></div>

    <div class="control-panel">
      <div class="panel-title">流动图片墙体效果</div>
      ...
      <div class="control-row">
        ...
        <input v-model.number="speed" type="range" min="0.5" max="20" step="0.5" />
        <span class="row-value">{{ speed.toFixed(1) }}</span>
      </div>
      <div class="control-row">
        <span class="row-label">重复次数</span>
        <input v-model.number="repeatX" type="range" min="2" max="40" step="1" />
      ...
      <p class="hint">箭头纹理沿墙体水平流动；开启放置后单击地图可移动位置，参数修改实时生效。</p>
...
<style scoped>
.wall-image-trail-shell { position: relative; width: 100%; height: 100%; min-height: 320px; overflow: hidden; border-radius: 8px; background: #152b4c; }
